...
begin_initialization {
 ...
 double eps0 = 1;         // Permittivity of space
  ...
  double taui    = 1000/wpe_wce;      // simulation wci's to run
  double amp = 0.3;
  ...
  double Lx  = 400.0/sqrt(mi_me)*di; // size of box in x dimension
  double Ly  = 100.0/sqrt(mi_me)*di;     // size of box in y dimension
  double Lz  = 100.0/sqrt(mi_me)*di; // size of box in z dimension
...
#define JZ_PERT_2 DJZ_2(-1,1,0.4) + DJZ_2(-1,-2,2.56) + DJZ_2(2,-3,4.19)
  ...
  set_region_field( everywhere, EX_PERT_2, EY_PERT_1, 0, BX_PERT_1, BY_PERT_2, b0);
  ...
  repeat ( Ne/nproc() ) {
   ...
   vx = normal(rng(0), 0, vthe) + UX_PERT_1 - JX_PERT_2*0.5 ;
   vy = normal(rng(0), 0, vthe) - JY_PERT_1*0.5 + UY_PERT_2 ;
   vz = normal(rng(0), 0, vthe) - JZ_PERT_1*0.5 -JZ_PERT_2*0.5;  // add drift
   ...
   vx = normal(rng(0), 0, vthe) + UX_PERT_1 + JX_PERT_2*0.5;
   vy = normal(rng(0), 0, vthe) + JY_PERT_1*0.5 + UY_PERT_2 ;
   vz = normal(rng(0), 0, vthe) + JZ_PERT_1*0.5 +JZ_PERT_2*0.5 ;
   v2 = vx*vx + vy*vy + vz*vz;
  ...
  sim_log( "Finished loading particles" );
	...
	*/
    ...
    pythonNames.push_back("insitu.py");
